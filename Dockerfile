...
ARG ubuntu_repo=https://mirror.sjtu.edu.cn/ubuntu-cdimage/releases/18.04.5/release	
ARG ubuntu_cd_name=ubuntu-18.04.5-server-arm64.iso	
ARG ubuntu_url=${ubuntu_repo}/${ubuntu_cd_name}

ARG python_pkg=Python-3.7.5.tgz
ARG python_url=https://mirrors.huaweicloud.com/python/3.7.5/${python_pkg}	   
...
RUN cd ${HOME}									&& \
    ...
    tar -zxvf ${python_pkg}							&& \
    cd ${python_pkg%.*}								&& \
    ./configure --prefix=/usr/local/${python_pkg%.*} --enable-shared		&& \
    make -j8									&& \
    make install								&& \
    cd ${HOME}									&& \
    rm -rf ${python_pkg%.*}*
